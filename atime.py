...
if 'admin_authenticated' in st.session_state and st.session_state['admin_authenticated']:
    admin_tab = st.sidebar.radio("Admin Functions", ["Manage Teams", "Manage Players", "Download Data", "Reset Data"])
...
''', unsafe_allow_html=True)
    ...
    if not active_item:
        ...
        st.image("https://i.ibb.co/SDSv8zQz/TITLE-SPONSOR.gif", caption="No item is currently available for bidding.", use_container_width=True)
    else:
        item_id, item_name, item_rating, item_category, item_nationality, item_image_url, item_base_price, current_bid, is_active, winner, unsold_timestamp = active_item
